...
import { IExpandRecord } from "@apitable/widget-sdk";
...
// 根据地址获取去高德地图定位点
export const getRcoresLocationAsync = (
  plugins: IPlugins | undefined,
  records: ISimpleRecords,
) => {
...
// 根据地址数组获取去高德地图定位点
export const getLocationArrayAsync = (
  plugins: IPlugins | undefined,
  records: any,
) => {
  if (!plugins) {
...
export const comparedMapRecords = (
  mapRecords: ISimpleRecords[],
  mapRecordsCache: ISimpleRecords[],
) => {
...
export const creatMarker = (
  ...
  marker.on("mouseover", () => {
    infoWindow.setContent(
      `<div class="infowindowContent" ><h1>${record.title}</h1><p>${record.address}</p></div>`,
    );
